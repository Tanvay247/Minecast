...
# Temporarily disable weights_only restriction
...
torch.load = _original_torch_load  # restore immediately
...
def caption_embedding(frames):
    inputs = blip_processor(
        images=frames[0], return_tensors="pt"
    ).to(device)
...
def audio_embedding(video_path: str):
    ...
    with torch.no_grad():
        emb = clap_model.get_audio_embedding_from_data(x=waveform)
